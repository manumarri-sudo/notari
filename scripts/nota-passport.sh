#!/usr/bin/env bash
#
# Nota Change Control - GitHub Action wrapper.
#
# Runs `nota verify`, turns the Change Passport into a PR-visible summary +
...
#
# Inputs (environment):
#   NOTA_HEAD          ref to verify against the contract base (default HEAD)
#   NOTA_PASSPORT_DIR  where the published passport.{json,md} are copied
#                       (default .nota); the verdict is NOT trusted from here
#   NOTA_STRICT        "true" (default) requires a signed perimeter + contract
#   NOTA_FAIL_ON_BLOCK "true" to exit 1 on BLOCK (default true)
#   NOTA_BLOCK_ON_REVIEW "true" treats NEEDS_REVIEW as BLOCK for the Status
#                       Check state + job exit (default false). The passport
#                       verdict itself is left unchanged.
#   NOTA_GATE_PUBKEYS  trusted gate PUBLIC key(s) (PEM/paths). When set, the
#                       passport signature MUST verify or the job fails closed.
#   NOTA_HEAD_SHA      commit SHA to attach the Status Check to (default: git HEAD)
...
PUBLISH_DIR="${NOTA_PASSPORT_DIR:-.nota}"
FAIL_ON_BLOCK="${NOTA_FAIL_ON_BLOCK:-true}"
STATUS_CONTEXT="nota/change-control"
...
# explicit candidate SHA; fall back to NOTA_HEAD, then HEAD.
# Validate explicit SHA inputs against hex pattern to prevent option injection.
if [[ -n "${NOTA_HEAD_SHA:-}" ]] && ! [[ "$NOTA_HEAD_SHA" =~ ^[0-9a-fA-F]{40}$ ]]; then
  echo "::error::NOTA_HEAD_SHA is not a valid 40-hex SHA: '${NOTA_HEAD_SHA}'"
  exit 2
fi
EVAL_REF="${NOTA_HEAD_SHA:-${NOTA_HEAD:-HEAD}}"
...
if [[ -n "$STRICT_FLAG" && "${GITHUB_EVENT_NAME:-}" == "pull_request" ]]; then
  if [[ "${NOTA_ALLOW_PULL_REQUEST_TRIGGER:-false}" != "true" ]]; then
    echo "::error::strict mode requires pull_request_target (not pull_request). The pull_request event runs the PR's workflow, so a PR can remove Nota entirely. See docs/SECURITY-MODEL.md for the secure template. Set NOTA_ALLOW_PULL_REQUEST_TRIGGER=true to override (NOT recommended)."
    exit 2
  else
    echo "::warning::running under pull_request with NOTA_ALLOW_PULL_REQUEST_TRIGGER=true — the caller workflow is candidate-controlled. A PR can remove this step."
  fi
fi
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/nota.XXXXXX")"
cleanup() { rm -rf "$WORK"; }
trap cleanup EXIT
...
set +e
nota verify $STRICT_FLAG --head "$EVAL_REF" --passport-dir "$WORK" \
...
#    error, never a pass.
if [[ "$NOTA_RC" != "0" && "$NOTA_RC" != "1" ]] || [[ ! -f "$PASSPORT_JSON" ]]; then
  echo "::error::nota verify failed to produce a verdict (rc=$NOTA_RC)"
...
# 3a. The verdict must be one Nota actually emits. A malformed / truncated /
...
if [[ "$NOTA_RC" != "$WANT_RC" || "$EXIT_CODE" != "$WANT_EXIT" ]]; then
  echo "::error::inconsistent verdict evidence (process rc=$NOTA_RC, verdict=$VERDICT, exit_code=$EXIT_CODE); failing closed"
  exit 2
fi
...
if [[ -n "${NOTA_GATE_PUBKEYS:-}" ]]; then
  if ! nota verify-passport "$PASSPORT_JSON" >/dev/null 2>"$WORK/sig.err"; then
  ...
  if [[ "${NOTA_ALLOW_UNSIGNED_EVIDENCE:-false}" == "true" ]]; then
    echo "::warning::strict mode with NOTA_ALLOW_UNSIGNED_EVIDENCE=true — the passport is UNSIGNED and cannot be independently re-verified."
  else
    echo "::error::strict mode requires a gate-signed passport. Set gate-key + gate-pubkeys, or set NOTA_ALLOW_UNSIGNED_EVIDENCE=true to accept report-grade unsigned evidence."
    exit 2
  fi
fi
...
# regardless of whether NOTA_HEAD_SHA was explicitly set. Resolve HEAD now
# so the binding is always checked (wrapper-scanner HIGH-1).
STATUS_SHA="${NOTA_HEAD_SHA:-$(git rev-parse HEAD 2>/dev/null || true)}"
...
# A real Nota status carries it in the description; a spoofed "success" posted
...
echo "Nota verdict: $VERDICT ($SAFE_REASONS)"
...
#     opening the summary. Rendered (and escaped) by Nota from the SAME passport
#     we just validated; best-effort, never affects the verdict.
if [[ "$VERDICT" != "PASS" ]]; then
  nota explain --passport "$PASSPORT_JSON" --format github 2>/dev/null || true
fi
...
if [[ "${NOTA_BLOCK_ON_REVIEW:-false}" == "true" && "$VERDICT" == "NEEDS_REVIEW" ]]; then
...
PY
  ...
  # Record the SHA + MAC we just posted so `nota verify-passport` can
  # cross-check that the status check on this commit was the one Nota wrote,
