...
#
# Notari's tool-name classifier is fast and right for namespaced MCP tools, but
...
CRITICAL_COMMAND_PATTERNS: Final[tuple[tuple[str, str, str], ...]] = (
    ...
    # Gate self-tamper: a write/delete/in-place-edit targeting Notari's own
    ...
    (
        ...
        "Change policy deliberately via `notari` commands, not by rewriting the files",
    ),
    # Gate self-tamper via environment variable: setting/exporting a var whose
    # purpose is to disable the host agent's hooks or Notari itself routes around
    ...
    (
        ...
        r"CURSOR_DISABLE_HOOKS?|NOTARI_(?:DISABLE|SKIP|BYPASS)\w*|DISABLE_NOTARI\w*)\s*=",
        "setting an env var that disables the gate's hooks",
        "Disabling hooks via an env var routes around the gate. Pause "
        "deliberately and on the record with `notari pause`, not a hidden export",
    ),
    # Gate self-disable via the CLI itself. An agent running `notari off` /
    # `notari pause` / `notari night` through its gated Bash tool is neutering the
    ...
    # allowlist can't downgrade it. A HUMAN typing `notari off` in their own
    ...
    (
        r"\bnotari\s+(?:off|pause|night)\b",
        "agent attempting to disable/pause the Notari gate via the CLI",
...
RAW_CRITICAL_COMMAND_PATTERNS: Final[tuple[tuple[str, str, str], ...]] = (
    ...
    # Gate self-disable via the LIBRARY, not the CLI. `python -c "from notari
    # import pause; pause.pause()"` (or notari.overnight.enable/turn_on) flips the
    # gate off through the state-mutation API, skipping the `notari off` CLI that
    ...
    # `notari off` in their own terminal (not hooked). (audit: 2nd-review gap #1.)
    (
        r"(?:from\s+notari\s+import\s+(?:pause|overnight)\b"
        r"|import\s+notari\.(?:pause|overnight)\b"
        r"|\bpause\.pause\s*\("
        r"|\bovernight\.(?:enable|turn_on)\s*\()",
        "library call disabling/pausing the Notari gate (pause/overnight API)",
        "Disabling the gate is a human-only action; run `notari off` in your own "
    ...
    # QUOTED (`> \"$HOME/.notari/pause.json\"`), which masking would blank - so
...
def _user_bash_allowlist() -> tuple[re.Pattern[str], ...]:
    ...
    Read from `[bash] allowlist = [...]` in ~/.notari/config.toml. Each entry is
...
# a string/glob/regex operation. `notari verify` calls evaluate_diff() to learn
...
# BLOCK verdict is composed from this evidence in notari.verify.
# ---------------------------------------------------------------------------
...
def classify_sensitive_surface(path: str) -> str | None:
    ...
    "notari_learning_state" - Notari's local advisory learning state
    ...
    if p in (".notari/lessons.json", ".notari/mistakes.jsonl"):
        return "notari_learning_state"
...
def evaluate_diff(diff_text: str, allowed_paths: Sequence[str]) -> DiffEvaluation:
    ...
    and sensitive surfaces touched - without rendering a verdict. notari.verify
    ...
    # Notari's own metadata dir (contract.json, exceptions.json, passport.*) is
    ...
    files = tuple(f for f in all_files if not f.path.startswith(".notari/"))
    ...
    surfaces: dict[str, list[str]] = {
        ...
        "gitconfig": [],
        "agent_instructions": [],
        "notari_learning_state": [],
    }

    # Sensitive-surface classification runs over the FULL diff (including the two
    # .notari learning-state files) so a lesson-store or agent-instruction edit
    # surfaces for review, even though .notari/ is exempt from the scope check
    # below (contract.json legitimately lives there). Only lessons.json /
    # mistakes.jsonl match a .notari category; contract/passport/keys return None.
    for f in all_files:
