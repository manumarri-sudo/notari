...
Notari's existing risk classifier + audit log + approvals + Touch ID flow
all work unchanged. The only adapter-layer work is the field-rename
between Cursor's input/output JSON and Notari's internal `decide()` core.
...
Hook contract (verified at https://cursor.com/docs/hooks):
  ...
  Config at `~/.cursor/hooks.json`:
    {
      ...
      "hooks": {
        "beforeShellExecution": [
          { "command": "notari cursor-hook", "type": "command", "timeout": 30 }
        ]
      }
    }

  Stdin (Cursor → Notari, one JSON line):
    { "command": "rm -rf /",
      "cwd": "/repo",
      "sandbox": false,
      "conversation_id": "...",
  ...
  Stdout (Notari → Cursor, one JSON line):
    { "permission": "deny",
      "agent_message": "rm -rf is blocked by Notari policy",
...
the user has enabled Auto-Run mode. Notari returns `"deny"` (not `"ask"`)
...
_EVENT_TOOL_NAME: dict[str, str] = {
    ...
    # observability hooks, not gate hooks - Notari ignores them by passing
    # through `allow`.
}


def _normalize_input(raw: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    """Cursor stdin → (tool_name, tool_input) for Notari's classifier.
...
def decide(tool_name: str, tool_input: Mapping[str, Any]) -> HookDecision:
    ...
    if base.permission == "ask":
        ...
        # user explicitly approves via `notari approve <token>` (the same
        ...
        return HookDecision(
            permission="deny",
            reason=body + " · approve via `notari approve <token>` to release",
            risk=risk,
...
def run_hook(stdin_text: str, audit: AuditLog | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "permission": "allow",
            "agent_message": f"notari: malformed cursor-hook input: {e}",
        }
    ...
    if session_id and session_id != "cursor":
        with contextlib.suppress(Exception):
            from notari.adapters.claude_code import _track_session
    ...
    if decision.permission == "allow" and not approval_token_used and session_id:
        with contextlib.suppress(Exception):
            from notari.adapters.claude_code import _taint_state_for
    ...
    if decision.permission != "allow":
        ...
        if decision.try_instead:
            response["user_message"] = (
                f"Notari blocked: {decision.what or tool_name}. Try: {decision.try_instead}"
            )
        else:
            response["user_message"] = f"Notari blocked: {decision.reason}"
...
def install_into_settings(settings_path: Any = None) -> tuple[Any, bool]:
    ...
    notari_entry = {"command": "notari cursor-hook", "type": "command", "timeout": 30}
    was_already = True
    for event_name in ("beforeShellExecution", "beforeMCPExecution", "beforeReadFile"):
        ...
        if not isinstance(existing_handlers, list):
            existing_handlers = []
        already = any(
            isinstance(h, dict) and h.get("command", "").startswith("notari ")
            for h in existing_handlers
        )
        if not already:
            existing_handlers.append(dict(notari_entry))
            was_already = False
...
def main() -> int:
    ...
    Wired to `notari cursor-hook` via the CLI module. Routes the audit
    log to <cwd>/.notari/audit.log.jsonl when the user has opted in to
    per-project mode (just like the Claude Code adapter).
    """
    from notari.adapters.claude_code import _default_load_hmac_key, _resolve_project_paths
