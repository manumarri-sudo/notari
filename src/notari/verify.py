"""`notari verify`: compare the diff to the contract and render a verdict.
...
+ scope + base commit), verify:
  ...
  3. subtracts any human exceptions logged in ``.notari/exceptions.json``,
...
from notari.contract import Contract, repo_root
from notari.errors import NotariError
from notari.perimeter import GATE_TAMPER_GLOBS, Perimeter, deny_hit
from notari.provenance import ProvenanceResult
...
def exceptions_path(root: Path) -> Path:
    return root / ".notari" / "exceptions.json"
...
class ScanLimits:
    ...
    def from_env(cls, env: dict[str, str]) -> ScanLimits:
        ...
        return cls(
            max_diff_bytes=_pos_int("NOTARI_MAX_DIFF_BYTES", DEFAULT_MAX_DIFF_BYTES),
            max_files=_pos_int("NOTARI_MAX_FILES", DEFAULT_MAX_FILES),
            git_timeout=_pos_int("NOTARI_GIT_TIMEOUT", DEFAULT_GIT_TIMEOUT),
        )
...
def verify(
    *,
    ...
    session_id: str = "notari-change-control",
    ...
    contract_prov = provenance_mod.verify_artifact(
        contract.to_dict(), root / ".notari" / "contract.sig", root, _env, strict=strict
    )
    ...
    expected_repo = (_env.get("NOTARI_REPO_ID") or _env.get("GITHUB_REPOSITORY") or "").strip()
    if strict and contract.repo and contract.repo != expected_repo:
        repo_msg = (
            ...
            if expected_repo
            else f"contract is bound to repo {contract.repo!r}, but the current repo is unknown (set NOTARI_REPO_ID)"
        )
    ...
    if strict and not contract.repo:
        ...
        repo_msg = (
            ...
            "unknown; strict requires --repo on the contract and NOTARI_REPO_ID / "
    ...
    raw_changed: set[str] = set()  # every touched path, both ends, incl. .notari/
    policed_changed: set[str] = set()  # scope/surface set: .notari/ excluded
    for cp in inventory:
    ...
    try:
        ...
        for i in range(0, len(parts) - 1, 2):
            ...
            if not path or path.startswith(".notari/"):
                continue
    ...
    # Gate-tamper over EVERY touched path (both rename endpoints, incl. .notari/),
    ...
    # Cursor rules) poison the next agent, and Notari's own learning state is
    ...
    _ALWAYS_REVIEW = {"agent_instructions", "notari_learning_state"}
