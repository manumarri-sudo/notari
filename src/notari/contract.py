"""Change contract: the human-approved task, captured at `notari begin`.

A contract is the anchor of Notari Change Control. Before an agent writes code,
a human records WHAT was approved (the task), WHERE it may touch (allowed
paths), and the base commit the work starts from. `notari verify` later compares
`git diff <base_commit> HEAD` against this record, so the contract is the fixed
point the whole verdict is measured against.

The file lives at ``<repo-root>/.notari/contract.json`` and is intentionally
plain JSON - it is committed nowhere by Notari itself, reviewable by a human, and
...
class ContractError(NotariError):
...
def repo_root(start: Path | None = None) -> Path:
    ...
    Falls back to `start` itself when not inside a git repo, so `notari begin`
...
def contract_path(root: Path) -> Path:
    return root / ".notari" / "contract.json"
...
class Contract:
    ...
    paths) that `notari verify` measures the diff against. An empty list means no
...
def load(root: Path | None = None) -> Contract:
    ...
    if not p.exists():
        msg = (
            f"no contract at {p}. Run `notari begin <task>` first to capture the "
...
def begin(
    ...
    *,
    ...
    `expires_in_days`, when set, records an expiry after which `notari verify
