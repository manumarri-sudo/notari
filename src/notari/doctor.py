"""`notari doctor` - first-run-friction killer.
...
def check_notari_version() -> CheckResult:
    return CheckResult(
        "notari",
        PASS,
        f"notari {__version__} (installed at {Path(__file__).resolve().parent})",
    )


def check_config(config_path: Path | None = None) -> tuple[CheckResult, NotariConfig | None]:
    ...
    if not p.exists():
        return (
            CheckResult(
                ...
                WARN,
                f"no config at {p}",
                fix="Run `notari init` to write a starter config.",
            ),
            None,
        )
    try:
        ...
        return (
            CheckResult(
                ...
                FAIL,
                f"{p}: {e}",
                fix="Fix the TOML errors above. Run `notari init --force` to reset.",
            ),
            None,
        )
...
def check_audit_log(audit_path: Path | None = None) -> CheckResult:
    ...
    if not os.access(parent, os.W_OK):
        return CheckResult(
            ...
            fix="chmod the directory or use NOTARI_LOG=path/to/your/audit.log.jsonl",
        )
    if p.exists():
...
def check_hmac_key() -> CheckResult:
    ...
    if not p.exists():
        return CheckResult(
            ...
            WARN,
            f"no key yet at {p}",
            fix="Will be auto-generated on first notari start / claude-hook invocation.",
        )
    mode = stat.S_IMODE(p.stat().st_mode)
    if mode & 0o077:
        return CheckResult(
    ...
    if p.stat().st_size != 32:
        return CheckResult(
            ...
            fix="Rotate the key: rm the file and let notari regenerate it.",
        )
...
def _hook_command_from_settings(
    ...
    Recognizes both bare `notari claude-hook` and any absolute-path form
    like `/Users/foo/.venv/bin/notari claude-hook`. The check is
    suffix-based on `notari claude-hook` so future-proofing across
    ...
    for block in pre_list:
        for h in block.get("hooks") or []:
            ...
            if cmd.endswith("notari claude-hook"):
                return cmd
    return None
...
def check_claude_hook_installed(
    ...
    if not p.exists():
        return CheckResult(
            ...
            fix="Run `notari claude-hook-install` once Claude Code is installed.",
        )
    try:
        ...
        return CheckResult(
            ...
            fix="Fix the JSON, then re-run notari claude-hook-install.",
        )
    ...
    if cmd is None:
        return CheckResult(
            ...
            fix="Run `notari claude-hook-install` to enable gating Claude Code's built-in tools.",
        )
    return CheckResult(
        "claude code hook",
        PASS,
...
def check_notari_on_path(
    settings_path: Path | None = None,
) -> CheckResult:
    """Verify that whichever notari the hook uses is actually executable.

    Three cases:
      1. Hook uses an absolute path that exists + is executable -> PASS
         (no PATH lookup needed because the hook bypasses PATH)
      2. Hook uses a bare `notari claude-hook` -> requires `notari` on PATH
    ...
    if cmd is None:
        # Falls back to the legacy "is notari on PATH?" check so a fresh
        # install without a hook still gets a useful answer.
        found = shutil.which("notari")
        if not found:
            return CheckResult(
                "notari on PATH",
                WARN,
                "the `notari` command is not on PATH and no hook is installed yet",
                fix="Install with `pipx install notari` or activate the venv, "
                "then run `notari onboard` or `notari claude-hook-install`.",
            )
        return CheckResult("notari on PATH", PASS, found)
    ...
    if binary_path.is_absolute():
        ...
        return CheckResult(
            ...
            fix="Re-run `notari claude-hook-install` from the correct venv, "
            "or edit the absolute path in ~/.claude/settings.json.",
        )

    # Bare command (e.g. "notari"). Falls back to PATH lookup.
    ...
    if not found:
        return CheckResult(
            ...
            fix="Either install notari on PATH (`pipx install notari`) OR "
            "re-run `notari claude-hook-install` from your venv to bake "
            "the absolute path into settings.json.",
        )
    return CheckResult("notari on PATH", PASS, f"hook resolves to {found}")


def check_upstream_executables(cfg: NotariConfig | None) -> list[CheckResult]:
...
def check_audit_chain_intact(audit_path: Path | None = None) -> CheckResult:
    ...
    key_path = default_path("key", env_override="NOTARI_KEY")
    if not key_path.exists():
        return CheckResult(
            "audit chain",
            WARN,
            "log exists but no HMAC key at default path; can't verify",
            fix="Set NOTARI_KEY to the key that wrote this log, or check ~/.notari/key.",
        )
    try:
        key = key_path.read_bytes()
        from notari.audit import verify_chain  # local import
        ...
        return CheckResult(
            ...
            FAIL,
            f"{p}: {e}",
            fix="The log may be corrupted. Stop notari and investigate.",
        )
    if failures:
        return CheckResult(
            "audit chain",
            ...
            fix=f"Inspect: notari audit verify --log {p}. Possible tampering.",
        )
...
def check_otel_dual_write() -> CheckResult:
    ...
    """
    try:
        from notari import otel as _otel
...
def run_doctor(
    ...
    report.add(check_notari_version())
    report.add(check_notari_on_path())
