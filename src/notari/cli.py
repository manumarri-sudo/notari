"""notari CLI.
  ...
  notari begin          write .notari/contract.json from the approved task
  notari verify         compare the diff to the contract, emit a verdict
  notari init           write a starter ~/.notari/config.toml
  notari tail           live-stream the audit log in a separate terminal
  notari audit verify   walk the HMAC chain on an existing log file
  notari audit show     pretty-print the log
...
from notari.doctor import run_doctor
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"notari {__version__}")
        raise typer.Exit()
...
def _main(
    version: Annotated[
        ...
        typer.Option(
            ...
            is_eager=True,
            help="Show the notari version and exit.",
        ),
    ] = None,
) -> None:
    """notari change control: verify AI-written diffs against the human-approved task."""
...
def begin_cmd(
    ...
    key: Annotated[
        ...
        typer.Option(
            ...
            "`notari verify --strict`: an unsigned contract is forgeable by the agent.",
        ),
    ] = None,
    expires_in: Annotated[
        ...
        typer.Option(
            "--expires-in",
            help="days until the approval lapses. After it, `notari verify --strict` "
    ...
    repo: Annotated[
        ...
        typer.Option(
            "--repo",
            help="bind this approval to a repository (owner/name). `notari verify "
    ...
    """Capture the human-approved task into .notari/contract.json.

    Records WHAT was approved, WHERE it may touch (--scope), and the current
    HEAD as the base commit. Sign it with --key so `notari verify --strict` can
    prove a human (not the agent) authored it. `notari verify` measures the diff
    ...
    if key is not None:
        ...
        provenance_mod.sign_artifact(
            contract.to_dict(), key.read_text(), root / ".notari" / "contract.sig"
        )
        signed = True
    ...
    if not signed:
        out.print(
            ...
            "`notari verify --strict` can establish provenance."
        )
...
def verify_cmd(
    ...
    passport_dir: Annotated[
        ...
        typer.Option(
            "--passport-dir",
            help="where to write passport.json + passport.md (default: <repo>/.notari).",
        ),
    ] = None,
    ...
    tampered perimeter also BLOCKs. Evidence goes to .notari/passport.{json,md}.
    """
    import os
    ...
    if not strict:
        console.print(
            ...
            "([bold]NOTARI_APPROVER_PUBKEYS[/bold]). See docs/SECURITY-MODEL.md."
        )

    gate_pem: str | None = None
    if sign_key is not None:
        gate_pem = sign_key.read_text()
    elif os.environ.get("NOTARI_GATE_KEY"):
        gate_pem = os.environ["NOTARI_GATE_KEY"]

    if write_passport:
        out_dir = passport_dir or (root / ".notari")
...
def keygen_cmd(
    ...
    the .pub goes in .notari/approvers/ or NOTARI_APPROVER_PUBKEYS so the gate can
...
def guard_cmd(
    ...
    Builds (or re-signs) .notari/perimeter.json from --allow / --forbid, signs it
    into .notari/perimeter.sig with your approver key, and reminds you to publish
    the matching public key. After this, `notari verify --strict` enforces the
    ...
    except (OSError, NotariError) as e:
...
def verify_passport_cmd(
    ...
    --gate-key files and/or the NOTARI_GATE_PUBKEYS env). A passport with no
    ...
    env_val = os.environ.get("NOTARI_GATE_PUBKEYS", "")
    ...
    if not gate_keys:
        console.print(
            "[red]no trusted gate public keys[/red] (pass --gate-key or set NOTARI_GATE_PUBKEYS)"
        )
...
def explain_cmd(
    passport_file: Annotated[
        ...
        typer.Option(
            "--passport",
            help="passport.json to explain (default: .notari/passport.json, then ./passport.json).",
        ),
    ] = None,
    fmt: Annotated[
        str,
    ...
    compact agent surfaces (same output as `notari fix-prompt` / `agent-brief`).
    """
    ...
    candidates = (
        ...
        else [Path(".notari/passport.json"), Path("passport.json")]
    )
...
def _emit_fix_prompt(passport_file: Path | None) -> None:
    """Shared by `notari fix-prompt` and `notari explain --fix-prompt`."""
    from notari import teach as teach_mod
...
# The Notari action pinned to an immutable commit SHA. A mutable tag (@v0) lets
# whoever controls the tag swap the gate's code, and `notari status` (readiness)
...
_NOTARI_ACTION_PIN = (
    "manumarri-sudo/notari@2a38304891b6f3ec96a777f186bb02dfb7d4b664"  # 0.3.4 release action
)
...
# The Action installs Notari in isolated mode (python -I) and reads the checkout
...
def init_cmd(
    ...
    wf = root / ".github" / "workflows" / "notari-change-control.yml"
...
def frameworks_cmd(
            ...
            data=[
                {
                    ...
                    "code": c.code,
                    "title": c.title,
                    "evidence_events": list(c.notari_event_types),
    ...
    if not controls:
        ...
        raise typer.Exit(code=1)
    out.print(
        f"[bold]Notari produces audit evidence for {len(controls)} controls "
        f"across {len(by_std)} frameworks.[/bold]"
    )
    out.print("[dim]Each line: what Notari records, and how an auditor would test it.[/dim]\n")
    for std in sorted(by_std):
        out.print(f"[bold cyan]{std}[/bold cyan]")
        for c in by_std[std]:
            out.print(f"  [bold]{c.code}[/bold]  {c.title}")
            out.print(f"    [dim]evidence:[/dim] {', '.join(c.notari_event_types)}")
            if c.auditor_sampling:
                out.print(f"    [dim]auditor samples by:[/dim] {c.auditor_sampling}")
        out.print("")
    out.print(
        "[dim]→ `notari audit export --pack` turns your real audit log into a "
...
def roster_cmd(
    ...
    nothing is written.
    """
    from notari import roster as roster_mod
    from notari.receipt import load_audit_events
...
def git_hook_cmd(ctx: typer.Context) -> None:
    ...
    these to notari.githook.prepare_commit_msg.
    """
    from notari.githook import prepare_commit_msg
...
def commit_hook_install(
    ...
    Safe to re-run. Refuses to overwrite a non-Notari prepare-commit-msg.
    """
    from notari.githook import install_hook
    ...
    if already:
        ...
        console.print(
            "  every `git commit` in this repo will now pre-fill a Notari "
...
def commit_hook_uninstall(
    ...
    from notari.githook import uninstall_hook
...
def integrate_cmd(
    ...
    remove: Annotated[
        bool,
        typer.Option("--remove", help="remove the Notari snippet instead of installing"),
    ] = False,
) -> None:
    """Teach your coding agent how to query Notari data via its existing LLM.
    ...
    deterministic `notari` commands your agent can run when you ask about
    agent activity. Idempotent; safe to re-run after Notari upgrades.

    No LLM ships in Notari itself. Your existing coding agent does the asking.

    Examples:
      notari integrate            # auto-detect agents and prompt for which to set up
      notari integrate list       # show what's supported + currently installed
      notari integrate claude-code           # install for Claude Code (project scope)
      notari integrate claude-code --global  # install for Claude Code (user scope)
      notari integrate cursor                # install for Cursor in current repo
      notari integrate claude-code --remove  # uninstall
    """
    ...
    if agent == "list":
        ...
        out.print(
            "\n[dim]run `notari integrate <name>` to install the rules snippet.[/dim]",
        )
        return
    ...
    if agent == "auto":
        ...
        if not found:
            ...
            out.print("  run `notari integrate list` to see what's supported.")
            raise typer.Exit(code=1)
        if len(found) == 1:
            agent = found[0].name
        else:
            ...
            for cand in found:
                out.print(f"  notari integrate {cand.name}   ({cand.label})")
            raise typer.Exit(code=0)

    integ = get_integration(agent)
    if integ is None:
        out.print(f"[red]unknown agent:[/red] {agent}")
        out.print("  run `notari integrate list` to see supported agents.")
        raise typer.Exit(code=1)

    if remove:
        path, removed = uninstall(integ, global_scope=global_scope)
        if removed:
            out.print(f"[green]removed[/green] Notari snippet from {path}")
        else:
...
def scan_prompts_cmd(
    ...
    See `notari scan-secrets` for the hard-block secret detector.
    """
    from notari.prompt_injection import scan as pi_scan
...
def onboard_cmd(
    ...
    Replaces the placeholder values from `notari init` with a guided flow
    ...
    """
    from notari.onboard import run as run_onboard
...
def kpis_cmd(
    ...
    console.print(
        f"\n[bold]notari kpis[/bold] [dim]({window_label}, {kpis.n_events} events)[/dim]\n",
    )
    ...
    console.print(
        ...
        f"{kpis.n_overrides}  [dim](approved one-shot via notari approve)[/dim]",
    )
    console.print()
...
def _hmac_key() -> bytes:
    """Load the HMAC signing key from ~/.notari/key, or generate on first run.
    ...
    p = default_path("key", env_override="NOTARI_KEY")
...
def night_cmd(
    ...
    flip on before bed:    notari night
    flip on for 4 hours:   notari night on --hours 4
    flip off in morning:   notari day
    check current state:   notari night status
    """
    ...
    if cmd in ("on", ""):
        ...
        console.print(
            "[dim]run `notari day` to flip off sooner, or `notari night status` to check.[/dim]"
        )
        return
    ...
    if cmd == "off":
        ...
        if still_active:
            ...
            console.print(
                "[dim]edit ~/.notari/config.toml `[overnight] enabled = false` to fully disable, "
        ...
        if state.high_approved or state.critical_blocked:
            ...
            console.print(
                "[dim]run `notari audit show --since 12h` to review what was auto-approved.[/dim]"
            )
        return
    ...
    with contextlib.suppress(Exception):
        with AuditLog(path=default_audit_path(), hmac_key=_hmac_key()) as audit:
            audit.emit(
                ...
                session_id="notari-cli",
                agent_id="notari.pause",
                risk="low",
                payload=payload,
                force_fsync=True,
            )
...
def _human_tty_challenge(console: Console, action: str) -> bool:
    ...
    `notari off|pause|night` (policy.CRITICAL_COMMAND_PATTERNS), which denies an
...
def _require_disable_auth(
    console: Console,
    *,
    action: str = "turn the Notari gate OFF",
    ...
    `notari off|pause|night` is CRITICAL (policy.CRITICAL_COMMAND_PATTERNS), so an
    ...
    `notari off` unusable on every normal uv/pip install (the binding never
    ...
    Tests inject their verdict by monkeypatching `notari.touchid.authenticate`
    and/or `notari.cli._human_tty_challenge`.
    """
...
def off_cmd(
    ...
    """pause the gate - one command to turn notari OFF, bounded and logged.
    ...
    Unlike `notari night` (which auto-approves HIGH but still gates CRITICAL),
    `notari off` turns the gate FULLY off, including the destructive class.
    That is intentional: a half-off switch is what pushes people to
    --dangerously-skip-permissions, which leaves no trail at all.

      pause 1 hour:        notari off
      pause 30 minutes:    notari off --for 30m --reason "noisy refactor"
      turn back on:        notari on
      check state:         notari off status
    """
    from notari import pause as _pause
    ...
    # default, so a hijacked agent can't neuter Notari via its own CLI.
    ...
    console.print(
        "[bold yellow]notari gate OFF[/bold yellow] - all tool calls will be allowed and logged."
    )
    ...
    console.print(
        ...
        "gate_paused=true. run `notari on` to re-enable now.[/dim]"
    )
...
def on_cmd() -> None:
    ...
    if not was_paused:
        ...
        return
    console.print("[bold green]notari gate ON[/bold green] - full gating restored.")
    if state.allowed_count:
        console.print(
            f"recap: [bold]{state.allowed_count}[/bold] call(s) were allowed while paused. "
            "review them: [dim]notari audit show --since 24h[/dim] (look for gate_paused)."
        )
...
def pause_cmd(
    ...
    """alias for `notari off` - pause the gate (bounded + logged)."""
...
def init_config(
    ...
    """Write a starter notari config to ~/.notari/config.toml."""
    ...
    console.print("then: [bold]notari start[/bold]")
...
def tail(
    ...
    """Live-stream the audit log. Run this in a side terminal while Notari serves."""
    p = log_path or default_audit_path()
    if not p.exists():
    ...
    # ✓ ✗ ? ↳ ▸ · - that's the lexicon every Notari surface uses.
    risk_color = {
        "low": "green",
        "medium": "cyan",
        "high": "yellow",
...
def audit_export(
    ...
    output_dir: Annotated[
        Path | None,
        typer.Option("--out", "-o", help="output directory (default: ./notari-evidence-pack)"),
    ] = None,
    aiuc_1: Annotated[
        bool,
    ...
    Maps Notari's audit-event taxonomy to EU AI Act (Art 12 + 14 + 19),
    ...
    """
    from notari.exports import aggregate, render_html, render_markdown
    ...
    out_dir = output_dir or Path.cwd() / "notari-evidence-pack"
...
def _render_html_to_pdf(html_path: Path, pdf_path: Path) -> tuple[bool, str]:
    ...
    installed (cross-platform, slower, requires `pip install notari[pdf]`).
    """
    ...
    return False, (
        ...
        "Install Chrome/Brave/Edge/Chromium, OR `pip install notari[pdf]` "
...
def _try_weasyprint_pdf(html_path: Path, pdf_path: Path) -> tuple[bool, str]:
    """Fallback PDF path: weasyprint (requires `pip install notari[pdf]`)."""
    try:
        from weasyprint import HTML
    except ImportError:
        return False, "weasyprint not installed (pip install notari[pdf])"
    try:
...
def audit_repair(
    ...
    This rewrites historical audit entries. It is the only notari command that
    ...
    from notari.audit import _canon
    ...
    with AuditLog(path=p, hmac_key=key) as log:
        log.emit(
            ...
            session_id="notari-audit-repair",
            risk="high",
            payload={
                "by": "notari audit repair",
...
def audit_summary(
    ...
    log_path: Annotated[
        Path | None,
        typer.Option("--log", "-l", help="audit log path (default: $NOTARI_HOME/audit.log.jsonl)"),
    ] = None,
    ...
    duration `notari night` defaults to. The markdown format is Substack-
    ready: paste it into a daily note or post and it lays out cleanly.

    Example:

        notari audit summary --since 12h
        notari audit summary --since 7d --format json --out recap.json
        notari audit summary --since 1d --cwd ~/notari --format table
    """
    from notari.audit_summary import (
        compute_summary,
        load_events,
        parse_duration,
        render_json,
...
def doctor(
    config_path: Annotated[
        Path | None,
        typer.Option("--config", "-c", help="path to notari config"),
    ] = None,
) -> None:
    ...
    out.print()
    out.print("[bold]notari doctor[/bold]")
    out.print()
...
def claude_hook() -> None:
    ...
    (Bash, Edit, Write, ...) is gated by Notari before it executes.
    Reads JSON on stdin, writes JSON on stdout, exits 0.

    Install with:  notari claude-hook-install
    """
...
def claude_hook_install(
    ...
    """Idempotently merge the Notari hook into Claude Code's settings.json.

    Safe to re-run; if Notari is already installed at this matcher, it does
    nothing.
    """
...
def cursor_hook() -> None:
    ...
    call is gated by Notari before it executes. Reads JSON on stdin, writes
    JSON on stdout (Cursor's `permission` shape, NOT Claude Code's).

    Install with:  notari cursor-hook-install
    """
    from notari.adapters import cursor as cursor_adapter
...
def cursor_hook_install(
    ...
    """Idempotently merge the Notari hook into Cursor's hooks.json.

    Wires beforeShellExecution + beforeMCPExecution + beforeReadFile to
    `notari cursor-hook`. Safe to re-run; if Notari is already wired, no-op.
    Requires Cursor 1.7+.
    """
    from notari.adapters import cursor as cursor_adapter
...
def telemetry_on() -> None:
    ...
    Console().print(
        ...
        "  Inspect what gets sent at any time:  notari telemetry show\n"
        "  Turn off:                            notari telemetry off",
    )
...
def telemetry_show(
    ...
    """Print the JSON Notari *would* send.
...
def receipts_list(
    ...
    from notari.receipt import derive_from_events, load_audit_events
...
def receipts_show(
    ...
    from notari.receipt import derive_from_events, load_audit_events, narrate
...
def pins_list(
    ...
    """
    from notari.pinning import PinStore
...
def approve_token(
    token: Annotated[
        str | None,
        typer.Argument(
            help="approval token from a Notari notification "
    ...
    When Notari blocks a tool call, the user gets a notification with a
    short token. Running `notari approve <token>` marks that exact
    ...
    # --latest (or a bare `notari approve`) resolves to the most recently issued
...
def _emit_approve_audit(
    ...
    try:
        ...
        with AuditLog(path=default_audit_path(), hmac_key=key) as audit:
            audit.emit(
                event_type=event_type,
                session_id="notari-approve-cli",
                agent_id="notari.approve",
                risk="high",
                payload={
...
def approvals_list() -> None:
    ...
    # `notari approve <token>` consumes; printing it here let an agent read its
    # own pending token and self-approve (esp. with --no-biometric). The
    # operator confirms with `notari approve --latest` (Touch-ID gated) and never
    ...
    Console().print(table)
    Console().print(
        "[dim]approve the most recent with [bold]notari approve --latest[/bold] "
...
def _load_or_init_config_toml() -> tuple[Path, dict[str, object]]:
    """Read ~/.notari/config.toml as a mutable dict; init a minimal one if missing.
    ...
    still pass Pydantic validation - NotariConfig requires SessionConfig.
    """
    import sys as _sys
    ...
    from notari.config import default_config_path
    ...
    if "session" not in data:
        ...
        data["session"] = {"intent": "(autocreated by notari trust)", "scope": []}
    return p, data
...
def _write_config_toml(path: Path, data: dict[str, object]) -> None:
    ...
    enough for the small surface notari writes (trust / policy / [session])."""
    out: list[str] = []
...
def trust_list() -> None:
    ...
    if not paths:
        ...
        console.print(f"[dim]add with: notari trust add <path>  ({cfg_path})[/dim]")
        return
...
def trust_check(
    ...
    from notari.paths import is_trusted_cwd
