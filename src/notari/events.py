...
Each event written to ~/.notari/audit.log.jsonl carries a `type` field whose
...
# Gate pause (`notari off` / `notari on`) - the bounded, audited off switch.
...
# Change Control (CI/CD pull-request gate). `notari begin` captures the
# human-approved task into a contract; `notari verify` compares the diff to
