"""`nota doctor` - first-run-friction killer.
...
def check_nota_version() -> CheckResult:
    return CheckResult(
        "nota",
        PASS,
        f"nota {__version__} (installed at {Path(__file__).resolve().parent})",
    )


def check_config(config_path: Path | None = None) -> tuple[CheckResult, NotaConfig | None]:
    ...
    if not p.exists():
        return (
            CheckResult(
                ...
                WARN,
                f"no config at {p}",
                fix="Run `nota init` to write a starter config.",
            ),
            None,
        )
    try:
        ...
        return (
            CheckResult(
                ...
                FAIL,
                f"{p}: {e}",
                fix="Fix the TOML errors above. Run `nota init --force` to reset.",
            ),
            None,
        )
...
def check_audit_log(audit_path: Path | None = None) -> CheckResult:
    ...
    if not os.access(parent, os.W_OK):
        return CheckResult(
            ...
            fix="chmod the directory or use NOTA_LOG=path/to/your/audit.log.jsonl",
        )
    if p.exists():
...
def check_hmac_key() -> CheckResult:
    ...
    if not p.exists():
        return CheckResult(
            ...
            WARN,
            f"no key yet at {p}",
            fix="Will be auto-generated on first nota start / claude-hook invocation.",
        )
    mode = stat.S_IMODE(p.stat().st_mode)
    if mode & 0o077:
        return CheckResult(
    ...
    if p.stat().st_size != 32:
        return CheckResult(
            ...
            fix="Rotate the key: rm the file and let nota regenerate it.",
        )
...
def _hook_command_from_settings(
    ...
    Recognizes both bare `nota claude-hook` and any absolute-path form
    like `/Users/foo/.venv/bin/nota claude-hook`. The check is
    suffix-based on `nota claude-hook` so future-proofing across
    ...
    for block in pre_list:
        for h in block.get("hooks") or []:
            ...
            if cmd.endswith("nota claude-hook"):
                return cmd
    return None
...
def check_claude_hook_installed(
    ...
    if not p.exists():
        return CheckResult(
            ...
            fix="Run `nota claude-hook-install` once Claude Code is installed.",
        )
    try:
        ...
        return CheckResult(
            ...
            fix="Fix the JSON, then re-run nota claude-hook-install.",
        )
    ...
    if cmd is None:
        return CheckResult(
            ...
            fix="Run `nota claude-hook-install` to enable gating Claude Code's built-in tools.",
        )
    return CheckResult(
        "claude code hook",
        PASS,
...
def check_nota_on_path(
    settings_path: Path | None = None,
) -> CheckResult:
    """Verify that whichever nota the hook uses is actually executable.

    Three cases:
      1. Hook uses an absolute path that exists + is executable -> PASS
         (no PATH lookup needed because the hook bypasses PATH)
      2. Hook uses a bare `nota claude-hook` -> requires `nota` on PATH
    ...
    if cmd is None:
        # Falls back to the legacy "is nota on PATH?" check so a fresh
        # install without a hook still gets a useful answer.
        found = shutil.which("nota")
        if not found:
            return CheckResult(
                "nota on PATH",
                WARN,
                "the `nota` command is not on PATH and no hook is installed yet",
                fix="Install with `pipx install notax` or activate the venv, "
                "then run `nota onboard` or `nota claude-hook-install`.",
            )
        return CheckResult("nota on PATH", PASS, found)
    ...
    if binary_path.is_absolute():
        ...
        return CheckResult(
            ...
            fix="Re-run `nota claude-hook-install` from the correct venv, "
            "or edit the absolute path in ~/.claude/settings.json.",
        )

    # Bare command (e.g. "nota"). Falls back to PATH lookup.
    ...
    if not found:
        return CheckResult(
            ...
            fix="Either install nota on PATH (`pipx install notax`) OR "
            "re-run `nota claude-hook-install` from your venv to bake "
            "the absolute path into settings.json.",
        )
    return CheckResult("nota on PATH", PASS, f"hook resolves to {found}")


def check_upstream_executables(cfg: NotaConfig | None) -> list[CheckResult]:
...
def check_audit_chain_intact(audit_path: Path | None = None) -> CheckResult:
    ...
    key_path = default_path("key", env_override="NOTA_KEY")
    if not key_path.exists():
        return CheckResult(
            "audit chain",
            WARN,
            "log exists but no HMAC key at default path; can't verify",
            fix="Set NOTA_KEY to the key that wrote this log, or check ~/.nota/key.",
        )
    try:
        key = key_path.read_bytes()
        from nota.audit import verify_chain  # local import
        ...
        return CheckResult(
            ...
            FAIL,
            f"{p}: {e}",
            fix="The log may be corrupted. Stop nota and investigate.",
        )
    if failures:
        return CheckResult(
            "audit chain",
            ...
            fix=f"Inspect: nota audit verify --log {p}. Possible tampering.",
        )
...
def check_stale_pattern_stats() -> CheckResult:
    ...
    operator can run `nota suggestions cleanup` to remove them."""
    try:
    ...
    return CheckResult(
        ...
        fix="nota suggestions cleanup",
    )


def check_self_improvement_signals() -> CheckResult:
    """Surface the highest-severity `nota learn` suggestion as a
    ...
    if high:
        ...
        return CheckResult(
            ...
            fix=f"see all: nota learn  ·  top action: {top.paste_command}",
        )
    top = suggestions[0]
    return CheckResult(
        "self-improvement",
...
def check_otel_dual_write() -> CheckResult:
    ...
    """
    try:
        from nota import otel as _otel
...
def check_permission_decay() -> CheckResult:
    """Surface decayed Nota permissions if any exist."""
    try:
        from nota import decay as _decay  # local import; optional path
        ...
        return CheckResult(
            ...
            fix="Inspect ~/.nota/permissions.json manually.",
        )
    ...
    if decayed:
        ...
        return CheckResult(
            ...
            fix="Run `nota decay show` for the full list, "
            "`nota decay reaffirm <pattern>` to refresh, or "
            "`nota decay forget <pattern>` to retire.",
        )
    if approaching:
        return CheckResult(
            "permission decay",
...
def run_doctor(
    ...
    report.add(check_nota_version())
    report.add(check_nota_on_path())
