"""Autonomous learning loop. Nota reads its own audit data and updates
...
The non-negotiable architectural rules:
  ...
  4. Every update appends a timestamped line to ~/.nota/learning.log
...
# chance to confirm without having to run `nota suggestions promote`.
#
...
def _stats_path() -> Path:
    return Path(
        os.environ.get(
            "NOTA_PATTERN_STATS",
            str(Path.home() / ".nota" / "pattern_stats.json"),
        )
    ).expanduser()
...
def _suggestions_path() -> Path:
    return Path(
        os.environ.get(
            "NOTA_SUGGESTIONS",
            str(Path.home() / ".nota" / "suggestions.jsonl"),
        )
    ).expanduser()
...
def _log_path() -> Path:
    return Path(
        os.environ.get(
            "NOTA_LEARNING_LOG",
            str(Path.home() / ".nota" / "learning.log"),
        )
    ).expanduser()
...
# ---------------------------------------------------------------------------
# Override reader. `nota suggestions promote` writes blocks into
...
def append_suggestion(payload: dict[str, Any]) -> None:
    ...
    `nota suggestions promote <id>` is the apply path."""
    p = _suggestions_path()
    p.parent.mkdir(parents=True, exist_ok=True)
...
def log_event(line: str) -> None:
    """Timestamped append to ~/.nota/learning.log. The operator tails
...
def detect_loosen_candidate(p: PatternStats) -> dict[str, Any] | None:
    ...
    `nota suggestions promote <id>` to write the override.
    """
    ...
    return {
        ...
        "proposal": (
            ...
            "`nota suggestions promote {id}`. Never auto-applied."
        ),
...
def detect_auto_promote_candidate(p: PatternStats) -> dict[str, Any] | None:
    ...
    `nota suggestions promote` command later.
    ...
    return {
        ...
        "proposal": (
            ...
            "Reverse anytime with `nota suggestions revoke`."
        ),
        "in_flow": True,
...
def post_decision_update(
    ...
    try:
        ...
        # Test-only env var NOTA_LEARNING_STRICT raises; production stays soft.
...
_STALE_PATTERN_PREFIXES: tuple[str, ...] = ("approved one-shot via nota approve ",)
...
def find_stale_patterns(
    ...
    via nota approve <token-prefix>") rather than the original deny
...
def stats_summary(stats: dict[str, PatternStats] | None = None) -> dict[str, Any]:
    """Snapshot for `nota log` and dashboards. Pure read."""
    s = stats if stats is not None else load_stats()
    return {
        "n_patterns": len(s),
