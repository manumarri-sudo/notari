...
Each event written to ~/.nota/audit.log.jsonl carries a `type` field whose
...
# Gate pause (`nota off` / `nota on`) - the bounded, audited off switch.
...
# Change Control (CI/CD pull-request gate). `nota begin` captures the
# human-approved task into a contract; `nota verify` compares the diff to
