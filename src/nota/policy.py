...
#
# Nota's tool-name classifier is fast and right for namespaced MCP tools, but
...
CRITICAL_COMMAND_PATTERNS: Final[tuple[tuple[str, str, str], ...]] = (
    ...
    # Gate self-tamper: a write/delete/in-place-edit targeting Nota's own
    ...
    (
        ...
        r"\.nota/(?:config\.toml|key|overrides\.toml|pause\.json))",
        ...
        "Change policy deliberately via `nota` commands, not by rewriting the files",
    ),
    # Gate self-tamper via environment variable: setting/exporting a var whose
    # purpose is to disable the host agent's hooks or Nota itself routes around
    ...
    (
        ...
        r"CURSOR_DISABLE_HOOKS?|NOTA_(?:DISABLE|SKIP|BYPASS)\w*|DISABLE_NOTA\w*)\s*=",
        "setting an env var that disables the gate's hooks",
        "Disabling hooks via an env var routes around the gate. Pause "
        "deliberately and on the record with `nota pause`, not a hidden export",
    ),
    # Gate self-disable via the CLI itself. An agent running `nota off` /
    # `nota pause` / `nota night` through its gated Bash tool is neutering the
    ...
    # allowlist can't downgrade it. A HUMAN typing `nota off` in their own
    ...
    (
        r"\bnota\s+(?:off|pause|night)\b",
        "agent attempting to disable/pause the Nota gate via the CLI",
        "Disabling the gate is a human-only action. If you are the human, run "
        "`nota off` yourself in your own terminal - it is not gated there.",
    ),
)
...
RAW_CRITICAL_COMMAND_PATTERNS: Final[tuple[tuple[str, str, str], ...]] = (
    ...
    # Gate self-disable via the LIBRARY, not the CLI. `python -c "from nota
    # import pause; pause.pause()"` (or nota.overnight.enable/turn_on) flips the
    # gate off through the state-mutation API, skipping the `nota off` CLI that
    ...
    # `nota off` in their own terminal (not hooked). (audit: 2nd-review gap #1.)
    (
        r"(?:from\s+nota\s+import\s+(?:pause|overnight)\b"
        r"|import\s+nota\.(?:pause|overnight)\b"
        r"|\bpause\.pause\s*\("
        r"|\bovernight\.(?:enable|turn_on)\s*\()",
        "library call disabling/pausing the Nota gate (pause/overnight API)",
        "Disabling the gate is a human-only action; run `nota off` in your own "
    ...
    # QUOTED (`> \"$HOME/.nota/pause.json\"`), which masking would blank - so
    ...
    (
        ...
        "Change policy via `nota` commands, not by rewriting the files.",
    ),
    (
        r"(?:^|[;&|]\s*)ruby\s+-e\b.*?"
...
def _user_bash_allowlist() -> tuple[re.Pattern[str], ...]:
    ...
    Read from `[bash] allowlist = [...]` in ~/.nota/config.toml. Each entry is
...
# a string/glob/regex operation. `nota verify` calls evaluate_diff() to learn
...
# BLOCK verdict is composed from this evidence in nota.verify.
# ---------------------------------------------------------------------------
...
def classify_sensitive_surface(path: str) -> str | None:
    ...
    "nota_learning_state" - Nota's local advisory learning state
                  (.nota/lessons.json, .nota/mistakes.jsonl). Advisory, never a
    ...
    if p in (".nota/lessons.json", ".nota/mistakes.jsonl"):
        return "nota_learning_state"
...
def evaluate_diff(diff_text: str, allowed_paths: Sequence[str]) -> DiffEvaluation:
    ...
    and sensitive surfaces touched - without rendering a verdict. nota.verify
    ...
    # Nota's own metadata dir (contract.json, exceptions.json, passport.*) is
    ...
    files = tuple(f for f in all_files if not f.path.startswith(".nota/"))
    ...
    # .nota learning-state files) so a lesson-store or agent-instruction edit
    # surfaces for review, even though .nota/ is exempt from the scope check
    # below (contract.json legitimately lives there). Only lessons.json /
    # mistakes.jsonl match a .nota category; contract/passport/keys return None.
    for f in all_files:
