...
def _nota_workflow_files(root: Path) -> list[Path]:
    """Workflow files that actually wire in Nota. Checks are scoped to these so a
    ``pull_request_target`` (or ``persist-credentials: false``) in an UNRELATED
    workflow can't launder a misconfigured Nota gate into an ENFORCED verdict
    ...
    return [
        ...
        if p.is_file() and re.search(r"nota", p.read_text(errors="replace"), re.I)
    ]


def _workflow_issue(text: str) -> str | None:
    """Return a one-line issue for a SINGLE workflow's text, or None if it is a
    safe Nota gate (pinned to a SHA, uses pull_request_target)."""
    ...
    if from_source or re.search(r"uses:\s*\./", text):
        return (
            ...
            "uses: manumarri-sudo/nota@<40-hex-release-sha>"
        )
    if not re.search(r"uses:\s*[\w.-]+/nota@", text):
        return "Nota referenced but not pinned to org/nota@<40-hex-release-sha>"
    issues: list[str] = []
    ...
    if not re.search(r"uses:\s*[\w.-]+/nota@[0-9a-f]{40}", text):
        issues.append("Nota action is not SHA-pinned (use a 40-hex commit SHA, not a tag)")
...
def _workflow_pinning(root: Path) -> Check:
    ...
    lets a PR modify the gate that judges it. A SHA pin (``uses: org/nota@<sha>``)
    runs trusted code the PR can't alter. The SAME file that runs Nota must also use
    ...
    if not wf_dir.is_dir():
        return Check(
            ...
            False,
            Level.BLOCKER,
            "no .github/workflows — the gate isn't wired into CI (run `nota init`)",
        )
    nota_files = _nota_workflow_files(root)
    if not nota_files:
        return Check("gate workflow", False, Level.BLOCKER, "no Nota step found in any workflow")
    last_issue = "no correctly-configured Nota workflow found"
    for p in nota_files:
...
def _workflow_hardening(root: Path) -> tuple[Check, ...]:
    ...
    if not nota_files:
        return ()
    # Scope hardening to the file(s) that run Nota, so an unrelated workflow's
    # persist-credentials/path can't satisfy the check for the gate.
    text = "\n".join(p.read_text(errors="replace") for p in nota_files)
    out: list[Check] = []
...
def assess(root: Path | None = None, env: dict[str, str] | None = None) -> ReadinessReport:
    ...
    if perimeter is None:
        return ReadinessReport(
            ...
            (Check("perimeter", False, Level.BLOCKER, "no signed perimeter (run `nota init`)"),),
        )
    ...
    gate_env = bool(env.get("NOTA_GATE_KEY", "").strip())
    checks.append(
        Check(
            "gate signing key",
            gate_env,
            Level.HARDENING,
            "NOTA_GATE_KEY present (passports are signed)"
            if gate_env
            else "NOTA_GATE_KEY not set — passports won't be gate-signed; "
