...
Storage: `~/.nota/session_approvals/<session_id>.json`. Atomic
...
def _dir() -> Path:
    base = os.environ.get("NOTA_SESSION_APPROVALS_DIR")
    if base:
        return Path(base)
    return Path.home() / ".nota" / "session_approvals"
...
def forget_session(session_id: str) -> None:
    """Wipe one session's approval cache. Called by `nota session reset`
