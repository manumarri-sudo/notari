...
When Nota blocks a tool call, the user gets a notification with a token.
Running `nota approve <token>` writes a short-lived approval record. The
...
Storage: $NOTA_HOME/approvals.json, mode 0o600. TTL default 10 minutes;
...
def _path() -> Path:
    ...
    return default_path("approvals.json", env_override="NOTA_APPROVALS_FILE")
...
class Approval:
    ...
    explicitly approves it (`nota approve <token>`, Touch-ID-gated where
    available). This separation is load-bearing: the gate auto-issues a
    token on every block so the notification can offer `nota approve`,
...
class ApprovalStore:
    ...
    def latest_pending(self) -> Approval | None:
        ...
        Backs `nota approve --latest`, so the operator can confirm the most
    ...
    def approve(self, token: str) -> Approval | None:
        """Mark a pending token approved (the user ran `nota approve <token>`).
