"""nota CLI.
  ...
  nota begin          write .nota/contract.json from the approved task
  nota verify         compare the diff to the contract, emit a verdict
  nota init           write a starter ~/.nota/config.toml
  nota tail           live-stream the audit log in a separate terminal
  nota audit verify   walk the HMAC chain on an existing log file
  nota audit show     pretty-print the log
...
from nota import decay as decay_mod
from nota import journal as journal_mod
from nota import telemetry as tel
from nota._version import __version__
from nota.adapters import claude_code as cc_adapter
from nota.audit import AuditLog, verify_chain
from nota.config import (
    default_audit_path,
    default_config_path,
    render_starter_config,
)
from nota.doctor import run_doctor
from nota.errors import NotaError

app = typer.Typer(
    add_completion=False,
    no_args_is_help=True,
    help="nota change control: verify AI-written diffs against the human-approved task.\n\n"
    "  nota begin      capture the approved task into .nota/contract.json\n"
    "  nota verify     compare the diff to the contract, emit PASS / NEEDS_REVIEW / BLOCK\n"
    "  nota onboard    first-run interactive setup (detects agents, installs hooks)\n"
    "  nota approve    go-ahead a blocked call (run from a notification)\n"
    "  nota audit      review what got blocked / allowed / asked\n"
    "  nota receipts   per-session did / changed / uncertain / to-verify\n"
    "  nota bridge     A2A handoff edges between agents\n"
    "  nota trifecta   exposure tracking (untrusted input + private data + exfil)\n"
    "  nota pins       tool description pins (anti-poisoning, anti-rug-pull)\n"
    "  nota approvals  list / revoke pending approval tokens\n"
    "  nota decay      permissions that erode without reinforcement\n"
    "  nota doctor     diagnose the install\n",
)


def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"nota {__version__}")
        raise typer.Exit()
...
def _main(
    version: Annotated[
        ...
        typer.Option(
            ...
            is_eager=True,
            help="Show the nota version and exit.",
        ),
    ] = None,
) -> None:
    """nota change control: verify AI-written diffs against the human-approved task."""
...
def begin_cmd(
    ...
    key: Annotated[
        ...
        typer.Option(
            ...
            "`nota verify --strict`: an unsigned contract is forgeable by the agent.",
        ),
    ] = None,
    expires_in: Annotated[
        ...
        typer.Option(
            "--expires-in",
            help="days until the approval lapses. After it, `nota verify --strict` "
    ...
    repo: Annotated[
        ...
        typer.Option(
            "--repo",
            help="bind this approval to a repository (owner/name). `nota verify "
    ...
    """Capture the human-approved task into .nota/contract.json.

    Records WHAT was approved, WHERE it may touch (--scope), and the current
    HEAD as the base commit. Sign it with --key so `nota verify --strict` can
    prove a human (not the agent) authored it. `nota verify` measures the diff
    ...
    if key is not None:
        ...
        provenance_mod.sign_artifact(
            contract.to_dict(), key.read_text(), root / ".nota" / "contract.sig"
        )
        signed = True
    ...
    if not signed:
        out.print(
            ...
            "`nota verify --strict` can establish provenance."
        )
    ...
    out.print("  next: let the agent work, then run [bold]nota verify[/bold]")
...
def verify_cmd(
    ...
    passport_dir: Annotated[
        ...
        typer.Option(
            "--passport-dir",
            help="where to write passport.json + passport.md (default: <repo>/.nota).",
        ),
    ] = None,
    ...
    sign_key: Annotated[
        ...
        typer.Option(
            ...
            "Falls back to the NOTA_GATE_KEY env value.",
        ),
    ] = None,
) -> None:
    ...
    tampered perimeter also BLOCKs. Evidence goes to .nota/passport.{json,md}.
    """
    import os
    ...
    if not strict:
        console.print(
            ...
            "[bold]nota verify --strict[/bold] with approver keys pinned off-box "
            "([bold]NOTA_APPROVER_PUBKEYS[/bold]). See docs/SECURITY-MODEL.md."
        )

    gate_pem: str | None = None
    if sign_key is not None:
        gate_pem = sign_key.read_text()
    elif os.environ.get("NOTA_GATE_KEY"):
        gate_pem = os.environ["NOTA_GATE_KEY"]

    if write_passport:
        out_dir = passport_dir or (root / ".nota")
    ...
    # `nota lessons` can aggregate repeats. Never influences the verdict,
...
def keygen_cmd(
    ...
    the .pub goes in .nota/approvers/ or NOTA_APPROVER_PUBKEYS so the gate can
...
def guard_cmd(
    ...
    Builds (or re-signs) .nota/perimeter.json from --allow / --forbid, signs it
    into .nota/perimeter.sig with your approver key, and reminds you to publish
    the matching public key. After this, `nota verify --strict` enforces the
    ...
    out.print(
        ...
        "    commit it to [bold].nota/approvers/<name>.pub[/bold], or set it as the\n"
        "    [bold]NOTA_APPROVER_PUBKEYS[/bold] CI secret (stronger: a PR can't edit it)."
    )
...
def verify_passport_cmd(
    ...
    --gate-key files and/or the NOTA_GATE_PUBKEYS env). A passport with no
    ...
    env_val = os.environ.get("NOTA_GATE_PUBKEYS", "")
    ...
    if not gate_keys:
        console.print(
            "[red]no trusted gate public keys[/red] (pass --gate-key or set NOTA_GATE_PUBKEYS)"
        )
    ...
    if status_fingerprint is not None:
        ...
        if not fp_mac or len(fp_mac) < 12:
            out.print(
                "[red]✗ status-fingerprint MAC missing or too short (need ≥12 hex chars)[/red]"
            )
            raise typer.Exit(code=1)
...
def explain_cmd(
    passport_file: Annotated[
        ...
        typer.Option(
            "--passport",
            help="passport.json to explain (default: .nota/passport.json, then ./passport.json).",
        ),
    ] = None,
    fmt: Annotated[
        str,
    ...
    compact agent surfaces (same output as `nota fix-prompt` / `agent-brief`).
    """
    from nota import explain as explain_mod
    ...
    candidates = (
        ...
        else [Path(".nota/passport.json"), Path("passport.json")]
    )
    found = next((p for p in candidates if p is not None and p.exists()), None)
    if found is None:
        out.print(
            "[red]no passport found[/red] — run `nota verify` first, or point at "
...
def lessons_main(
    ...
    Nota turns every blocked AI PR into a lesson future agents can learn
    from: `nota verify` records structured mistake events locally (never
    code, diffs, or secret values), this command aggregates the repeats, and
    `nota lessons promote <id>` + `nota teach` write the human-approved
    ...
    if not patterns:
        out.print(
            ...
            "when `nota verify` returns BLOCK or NEEDS_REVIEW."
        )
        return
...
def lessons_promote_cmd(
    lesson_id: Annotated[str, typer.Argument(help="lesson id from `nota lessons`.")],
) -> None:
    """Promote a suggested lesson into the repo's lesson store (human-gated).

    Idempotent. Run `nota teach` afterwards to write promoted lessons into
    ...
    try:
        ...
        out.print(f"[red]unknown lesson id '{lesson_id}'[/red] — run `nota lessons` for the list.")
        raise typer.Exit(code=2) from None
    verb = "promoted" if newly else "already promoted"
    out.print(f"[green]✓[/green] {verb}: {lesson_id}")
    out.print(f"  {text}")
    out.print("  now run [cyan]nota teach[/cyan] to write it into agent instruction files.")
...
def teach_cmd(
    ...
    and .cursor/rules/nota-scope.mdc (plus .cursorrules when it already
    exists). Only the nota-lessons block is touched; user content outside
...
def _emit_fix_prompt(passport_file: Path | None) -> None:
    """Shared by `nota fix-prompt` and `nota explain --fix-prompt`."""
    from nota import teach as teach_mod
    ...
    out = Console()
    if found is None:
        out.print("[red]no passport found[/red] — run `nota verify` first, or pass --passport.")
        raise typer.Exit(code=2)
    try:
        passport = json.loads(found.read_text())
    except (OSError, json.JSONDecodeError) as e:
...
def _emit_agent_brief() -> None:
    """Shared by `nota agent-brief` and `nota explain --agent-brief`."""
    from nota import contract as contract_mod
    from nota import lessons as lessons_mod
    from nota import perimeter as perimeter_mod
    from nota import teach as teach_mod
    ...
    try:
        ...
        out.print(f"[red]no active contract[/red] — run `nota begin` first ({e}).")
        raise typer.Exit(code=2) from e
    perimeter = perimeter_mod.load(root)
    print(
...
def fix_prompt_cmd(
    passport_file: Annotated[
        Path | None,
        typer.Option("--passport", help="passport.json (default: .nota/passport.json)."),
    ] = None,
) -> None:
...
def agent_brief_cmd() -> None:
    ...
    of any agent session. Alias for `nota explain --agent-brief`.
    """
    _emit_agent_brief()
...
# The Nota action pinned to an immutable commit SHA. A mutable tag (@v0) lets
# whoever controls the tag swap the gate's code, and `nota status` (readiness)
...
_NOTA_ACTION_PIN = "manumarri-sudo/nota@4a853898bb18cc806b9a868ecb77bdabdb3257d8"  # v0
...
# candidate pip.py / sitecustomize.py / json.py cannot shadow trusted imports.
# The Action installs Nota in isolated mode (python -I) and reads the checkout
# only via git. persist-credentials: false keeps the write token out of that tree.
on:
  pull_request_target:
    branches: [main]
permissions:
...
jobs:
  change-control:
    ...
    steps:
      ...
      - uses: {_NOTA_ACTION_PIN}  # v0 — SHA-pinned; bump on release
        with:
          strict: "true"
          head: ${{{{ github.event.pull_request.head.sha }}}}
          head-sha: ${{{{ github.event.pull_request.head.sha }}}}
          checkout-path: _pr_checkout
          passport-dir: _pr_checkout/.nota
...
def init_cmd(
    ...
    for name, content, mode in (
        ...
        if p.exists() and force:
            p.unlink()
        fd = os.open(str(p), os.O_CREAT | os.O_EXCL | os.O_WRONLY, mode)
        try:
    ...
    wf = root / ".github" / "workflows" / "nota-change-control.yml"
    ...
    out.print(f"     [dim]gh secret set NOTA_GATE_KEY < {keys_dir / 'gate.pem'}[/dim]")
    out.print(f"     [dim]gh secret set NOTA_APPROVER_PUBKEYS < {keys_dir / 'approver.pub'}[/dim]")
    out.print("     [dim](better: keep the approver key on a YubiKey/HSM, never on disk)[/dim]")
    out.print(
        "  2. The workflow uses [bold]pull_request_target[/bold] and SHA-pinned actions (already set)."
    )
    out.print("  3. Make the [bold]nota/change-control[/bold] status check REQUIRED in branch")
    out.print("     protection on main (admin-bypass + force-push off).")
    out.print(
        "\n[bold]Now:[/bold] commit .nota/ (NOT .nota/keys/), then run [bold]nota status[/bold]."
    )
...
def frameworks_cmd(
    ...
    """Show the compliance-framework crosswalk (`nota frameworks`): every control Nota produces evidence for.
    ...
    real audit log into a shareable PDF/HTML pack, use `nota audit export --pack`.
    """

    from nota import exports
            ...
            data=[
                {
                    ...
                    "code": c.code,
                    "title": c.title,
                    "evidence_events": list(c.nota_event_types),
    ...
    if not controls:
        ...
        raise typer.Exit(code=1)
    out.print(
        f"[bold]Nota produces audit evidence for {len(controls)} controls "
        f"across {len(by_std)} frameworks.[/bold]"
    )
    out.print("[dim]Each line: what Nota records, and how an auditor would test it.[/dim]\n")
    for std in sorted(by_std):
        out.print(f"[bold cyan]{std}[/bold cyan]")
        for c in by_std[std]:
            out.print(f"  [bold]{c.code}[/bold]  {c.title}")
            out.print(f"    [dim]evidence:[/dim] {', '.join(c.nota_event_types)}")
            if c.auditor_sampling:
                out.print(f"    [dim]auditor samples by:[/dim] {c.auditor_sampling}")
        out.print("")
    out.print(
        "[dim]→ `nota audit export --pack` turns your real audit log into a "
...
def roster_cmd(
    log_path: Annotated[
        Path | None, typer.Option("--log", "-l", help="audit log to read (default: ~/.nota).")
    ] = None,
    ...
    nothing is written.
    """
    from nota import roster as roster_mod
    from nota.receipt import load_audit_events
...
def git_hook_cmd(ctx: typer.Context) -> None:
    ...
    these to nota.githook.prepare_commit_msg.
    """
    from nota.githook import prepare_commit_msg
...
def commit_hook_install(
    ...
    Safe to re-run. Refuses to overwrite a non-Nota prepare-commit-msg.
    """
    from nota.githook import install_hook
    ...
    if already:
        ...
        console.print(
            "  every `git commit` in this repo will now pre-fill a Nota "
...
def commit_hook_uninstall(
    ...
    from nota.githook import uninstall_hook
...
def insights_cmd(
    ...
    Goes deeper than `nota saves`: for each pattern, shows fire frequency,
    ...
    unusual hours.
    """
    from nota.insights import compute_insights, format_insights
    from nota.saves import parse_window
...
def integrate_cmd(
    ...
    remove: Annotated[
        bool,
        typer.Option("--remove", help="remove the Nota snippet instead of installing"),
    ] = False,
) -> None:
    """Teach your coding agent how to query Nota data via its existing LLM.
    ...
    deterministic `nota` commands your agent can run when you ask about
    agent activity. Idempotent; safe to re-run after Nota upgrades.

    No LLM ships in Nota itself. Your existing coding agent does the asking.

    Examples:
      nota integrate            # auto-detect agents and prompt for which to set up
      nota integrate list       # show what's supported + currently installed
      nota integrate claude-code           # install for Claude Code (project scope)
      nota integrate claude-code --global  # install for Claude Code (user scope)
      nota integrate cursor                # install for Cursor in current repo
      nota integrate claude-code --remove  # uninstall
    """
    ...
    if agent == "list":
        ...
        out.print("[bold]nota integrate — supported agents[/bold]\n")
        ...
        out.print(
            "\n[dim]run `nota integrate <name>` to install the rules snippet.[/dim]",
        )
        return
    ...
    if agent == "auto":
        ...
        if not found:
            ...
            out.print("  run `nota integrate list` to see what's supported.")
            raise typer.Exit(code=1)
        if len(found) == 1:
            agent = found[0].name
        else:
            ...
            for cand in found:
                out.print(f"  nota integrate {cand.name}   ({cand.label})")
            raise typer.Exit(code=0)

    integ = get_integration(agent)
    if integ is None:
        out.print(f"[red]unknown agent:[/red] {agent}")
        out.print("  run `nota integrate list` to see supported agents.")
        raise typer.Exit(code=1)

    if remove:
        path, removed = uninstall(integ, global_scope=global_scope)
        if removed:
            out.print(f"[green]removed[/green] Nota snippet from {path}")
        else:
    ...
    if status in ("installed", "refreshed"):
        out.print(
            "\n[dim]your coding agent will now know how to query Nota data when you ask.\n"
...
def saves_cmd(
    ...
    log_path: Annotated[
        Path | None,
        typer.Option("--log", "-l", help="audit log path (default: ~/.nota/audit.log.jsonl)"),
    ] = None,
    plain: Annotated[
        bool,
    ...
    """Show what Nota caught for you (verified counts + estimated savings).
    ...
    """
    from nota.saves import compute_saves, format_saves, parse_window
...
def scan_prompts_cmd(
    ...
    See `nota scan-secrets` for the hard-block secret detector.
    """
    from nota.prompt_injection import scan as pi_scan
...
def onboard_cmd(
    ...
    Replaces the placeholder values from `nota init` with a guided flow
    ...
    """
    from nota.onboard import run as run_onboard
...
def learn_cmd(
    ...
    each one. Nota never auto-applies learning to its own gate.
    ...
    """
    from nota.learn import analyze
...
def kpis_cmd(
    ...
    console.print(
        f"\n[bold]nota kpis[/bold] [dim]({window_label}, {kpis.n_events} events)[/dim]\n",
    )
    ...
    console.print(
        ...
        f"{kpis.n_overrides}  [dim](approved one-shot via nota approve)[/dim]",
    )
    console.print()
...
def _hmac_key() -> bytes:
    """Load the HMAC signing key from ~/.nota/key, or generate on first run.
    ...
    p = default_path("key", env_override="NOTA_KEY")
...
def night_cmd(
    ...
    flip on before bed:    nota night
    flip on for 4 hours:   nota night on --hours 4
    flip off in morning:   nota day
    check current state:   nota night status
    """
    from nota import overnight as ovn
    ...
    if cmd in ("on", ""):
        ...
        console.print(
            "[dim]run `nota day` to flip off sooner, or `nota night status` to check.[/dim]"
        )
        return
    ...
    if cmd == "off":
        ...
        if still_active:
            ...
            console.print(
                "[dim]edit ~/.nota/config.toml `[overnight] enabled = false` to fully disable, "
        ...
        if state.high_approved or state.critical_blocked:
            ...
            console.print(
                "[dim]run `nota audit show --since 12h` to review what was auto-approved.[/dim]"
            )
        return
...
@app.command("day")
def day_cmd() -> None:
    """flip overnight mode off. alias for `nota night off`."""
    from nota import overnight as ovn
...
def _human_tty_challenge(console: Console, action: str) -> bool:
    ...
    `nota off|pause|night` (policy.CRITICAL_COMMAND_PATTERNS), which denies an
...
def _require_disable_auth(
    console: Console,
    *,
    action: str = "turn the Nota gate OFF",
    ...
    `nota off|pause|night` is CRITICAL (policy.CRITICAL_COMMAND_PATTERNS), so an
    ...
    `nota off` unusable on every normal uv/pip install (the binding never
    ...
    Tests inject their verdict by monkeypatching `nota.touchid.authenticate`
    and/or `nota.cli._human_tty_challenge`.
    """
...
def off_cmd(
    ...
    """pause the gate - one command to turn nota OFF, bounded and logged.
    ...
    Unlike `nota night` (which auto-approves HIGH but still gates CRITICAL),
    `nota off` turns the gate FULLY off, including the destructive class.
    That is intentional: a half-off switch is what pushes people to
    --dangerously-skip-permissions, which leaves no trail at all.

      pause 1 hour:        nota off
      pause 30 minutes:    nota off --for 30m --reason "noisy refactor"
      turn back on:        nota on
      check state:         nota off status
    """
    from nota import pause as _pause
    ...
    # default, so a hijacked agent can't neuter Nota via its own CLI.
    ...
    console.print(
        "[bold yellow]nota gate OFF[/bold yellow] - all tool calls will be allowed and logged."
    )
    ...
    console.print(
        ...
        "gate_paused=true. run `nota on` to re-enable now.[/dim]"
    )


@app.command("on")
def on_cmd() -> None:
    """resume the gate - turn nota back ON. Logs a gate.resumed event with a
    ...
    if not was_paused:
        ...
        return
    console.print("[bold green]nota gate ON[/bold green] - full gating restored.")
    if state.allowed_count:
        console.print(
            f"recap: [bold]{state.allowed_count}[/bold] call(s) were allowed while paused. "
            "review them: [dim]nota audit show --since 24h[/dim] (look for gate_paused)."
        )


# Memorable aliases: `nota pause` / `nota resume` do the same as off / on.
@app.command("pause")
def pause_cmd(
    for_: Annotated[
        str,
    ...
    """alias for `nota off` - pause the gate (bounded + logged)."""
    off_cmd(for_=for_, reason=reason)


@app.command("resume")
def resume_cmd() -> None:
    """alias for `nota on` - resume the gate."""
    on_cmd()
...
def init_config(
    ...
    """Write a starter nota config to ~/.nota/config.toml."""
    ...
    console.print("then: [bold]nota start[/bold]")
...
def tail(
    ...
    """Live-stream the audit log. Run this in a side terminal while Nota serves."""
    p = log_path or default_audit_path()
    if not p.exists():
    ...
    # ✓ ✗ ? ↳ ▸ · - that's the lexicon every Nota surface uses.
    risk_color = {
        "low": "green",
        "medium": "cyan",
        "high": "yellow",
...
def audit_verify(
    ...
    if failures:
        ...
        console.print(
            "  if these failures pre-date nota 0.1.1, they may be from "
            "concurrent-write breaks fixed in 0.1.1.\n"
            "  to re-chain those entries: [bold]nota audit repair --legacy --yes[/bold]"
        )
        raise typer.Exit(code=2)
...
def audit_export(
    ...
    output_dir: Annotated[
        Path | None,
        typer.Option("--out", "-o", help="output directory (default: ./nota-evidence-pack)"),
    ] = None,
    aiuc_1: Annotated[
        bool,
    ...
    Maps Nota's audit-event taxonomy to EU AI Act (Art 12 + 14 + 19),
    ...
    """
    from nota.exports import aggregate, render_html, render_markdown
    ...
    if pack:
        ...
        from nota.exports import CONTROLS
    ...
    out_dir = output_dir or Path.cwd() / "nota-evidence-pack"
...
def _render_html_to_pdf(html_path: Path, pdf_path: Path) -> tuple[bool, str]:
    ...
    installed (cross-platform, slower, requires `pip install notax[pdf]`).
    """
    ...
    return False, (
        ...
        "Install Chrome/Brave/Edge/Chromium, OR `pip install notax[pdf]` "
...
def _try_weasyprint_pdf(html_path: Path, pdf_path: Path) -> tuple[bool, str]:
    """Fallback PDF path: weasyprint (requires `pip install notax[pdf]`)."""
    try:
        from weasyprint import HTML  # type: ignore[import-not-found]
    except ImportError:
        return False, "weasyprint not installed (pip install notax[pdf])"
    try:
...
def audit_repair(
    ...
    This rewrites historical audit entries. It is the only nota command that
    ...
    with AuditLog(path=p, hmac_key=key) as log:
        log.emit(
            ...
            session_id="nota-audit-repair",
            risk="high",
            payload={
                "by": "nota audit repair",
...
def audit_summary(
    ...
    log_path: Annotated[
        Path | None,
        typer.Option("--log", "-l", help="audit log path (default: $NOTA_HOME/audit.log.jsonl)"),
    ] = None,
    ...
    duration `nota night` defaults to. The markdown format is Substack-
    ready: paste it into a daily note or post and it lays out cleanly.

    Example:

        nota audit summary --since 12h
        nota audit summary --since 7d --format json --out recap.json
        nota audit summary --since 1d --cwd ~/nota --format table
    """
...
def doctor(
    config_path: Annotated[
        Path | None,
        typer.Option("--config", "-c", help="path to nota config"),
    ] = None,
) -> None:
    ...
    out.print()
    out.print("[bold]nota doctor[/bold]")
    out.print()
...
def claude_hook() -> None:
    ...
    (Bash, Edit, Write, ...) is gated by Nota before it executes.
    Reads JSON on stdin, writes JSON on stdout, exits 0.

    Install with:  nota claude-hook-install
    """
...
def claude_hook_install(
    ...
    """Idempotently merge the Nota hook into Claude Code's settings.json.

    Safe to re-run; if Nota is already installed at this matcher, it does
    nothing.
    """
...
def cursor_hook() -> None:
    ...
    call is gated by Nota before it executes. Reads JSON on stdin, writes
    JSON on stdout (Cursor's `permission` shape, NOT Claude Code's).

    Install with:  nota cursor-hook-install
    """
    from nota.adapters import cursor as cursor_adapter
...
def cursor_hook_install(
    ...
    """Idempotently merge the Nota hook into Cursor's hooks.json.

    Wires beforeShellExecution + beforeMCPExecution + beforeReadFile to
    `nota cursor-hook`. Safe to re-run; if Nota is already wired, no-op.
    Requires Cursor 1.7+.
    """
    from nota.adapters import cursor as cursor_adapter
...
def telemetry_on() -> None:
    ...
    Console().print(
        ...
        "  Inspect what gets sent at any time:  nota telemetry show\n"
        "  Turn off:                            nota telemetry off",
    )
...
def telemetry_show(
    ...
    """Print the JSON Nota *would* send.
...
def journal_save(
    ...
    if session_id:
        from nota.journal import _check_session_drift, _emit_session_close
    ...
    if transcript is None:
        Console(stderr=True).print(
            "[dim]nota journal save: no transcript provided; "
...
def decay_show(
    ...
    (the default risk fires) until you run `nota decay reaffirm`.
    """
    out = Console()
    ...
    if all_ and healthy:
        ...
        out.print(
            f"[dim]+ {len(healthy)} healthy permission(s) (nota decay show --all to see)[/dim]"
        )
...
@app.command()
def version() -> None:
    """Print the nota version."""
    console.print(f"nota {__version__}")
...
def receipts_list(
    ...
    from nota.receipt import derive_from_events, load_audit_events
...
def receipts_show(
    ...
    from nota.receipt import derive_from_events, load_audit_events, narrate
...
def pins_list(
    ...
    """
    from nota.pinning import PinStore
...
def approve_token(
    token: Annotated[
        str | None,
        typer.Argument(
            help="approval token from a Nota notification "
    ...
    When Nota blocks a tool call, the user gets a notification with a
    short token. Running `nota approve <token>` marks that exact
    ...
    # --latest (or a bare `nota approve`) resolves to the most recently issued
    ...
    if not skip_biometric:
        ...
        if touchid.is_available():
            ...
            # `nota approvals list` and run `nota approve` in its own context,
...
def _emit_approve_audit(
    ...
    try:
        ...
        with AuditLog(path=default_audit_path(), hmac_key=key) as audit:
            audit.emit(
                event_type=event_type,
                session_id="nota-approve-cli",
                agent_id="nota.approve",
                risk="high",
                payload={
...
def approvals_list() -> None:
    ...
    # `nota approve <token>` consumes; printing it here let an agent read its
    # own pending token and self-approve (esp. with --no-biometric). The
    # operator confirms with `nota approve --latest` (Touch-ID gated) and never
    ...
    Console().print(table)
    Console().print(
        "[dim]approve the most recent with [bold]nota approve --latest[/bold] "
...
def _load_or_init_config_toml() -> tuple[Path, dict[str, object]]:
    """Read ~/.nota/config.toml as a mutable dict; init a minimal one if missing.
    ...
    still pass Pydantic validation - NotaConfig requires SessionConfig.
    """
    import sys as _sys
    ...
    from nota.config import default_config_path
    ...
    if "session" not in data:
        ...
        data["session"] = {"intent": "(autocreated by nota trust)", "scope": []}
    return p, data
...
def _write_config_toml(path: Path, data: dict[str, object]) -> None:
    ...
    enough for the small surface nota writes (trust / policy / [session])."""
    out: list[str] = []
...
def trust_list() -> None:
    ...
    if not paths:
        ...
        console.print(f"[dim]add with: nota trust add <path>  ({cfg_path})[/dim]")
        return
...
def trust_check(
    ...
    from nota.paths import is_trusted_cwd
    ...
    if is_trusted_cwd(resolved):
        ...
        console.print(f"  [dim]add with: nota trust add {resolved}[/dim]")
        raise typer.Exit(code=1)
...
def suggestions_list(
    ...
    from nota.learning import read_suggestions
    ...
    for s in out:
        ...
        if s.get("type") == "loosening_candidate":
            console.print(
                f'          [bold]apply:[/bold] nota suggestions promote "{_suggestion_key(s)}"'
            )
...
def suggestions_show(
    key: Annotated[
        str,
        typer.Argument(help="suggestion key from `nota suggestions list` (type:pattern)"),
    ],
) -> None:
...
def suggestions_promote(
    ...
    `~/.nota/overrides.toml` with the given TTL. The operator's
    ...
    overrides_path = Path(
        os.environ.get(
            "NOTA_OVERRIDES",
            str(Path.home() / ".nota" / "overrides.toml"),
        )
    ).expanduser()
    ...
    from nota.learning import append_suggestion, log_event
    ...
    promo = {
        ...
        "ttl_days": ttl_days,
        "promoted_via": "nota suggestions promote",
...
def suggestions_cleanup(
    ...
    (e.g. `Bash:approved one-shot via nota approve aBc12`). This
    command cleans those up. Real patterns are untouched.

    Idempotent: a second invocation after the first removes nothing.
    """
    from nota.learning import cleanup_stale_patterns, find_stale_patterns
...
# operator can SEE what Nota is doing as it does it.
...
def log_cmd(
    ...
    show_suggestions: Annotated[
        ...
        typer.Option(
            ...
            help="also tail ~/.nota/suggestions.jsonl",
        ),
    ] = True,
) -> None:
    """Show the learner's recent activity. With --follow, streams new
    entries in real time so you can watch Nota update itself.
    """
    from nota.learning import _log_path, _suggestions_path
