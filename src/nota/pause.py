...
`nota off` (alias `nota pause`) flips the gate fully off. The honesty
...
Difference from overnight mode (`nota night`): overnight auto-approves
...
State file: $NOTA_HOME/pause.json (mode 0o600).
"""
...
def _state_path() -> Path:
    ...
    return default_path("pause.json", env_override="NOTA_PAUSE_FILE")
...
class PauseState:
    ...
    `paused`        True iff the operator ran `nota off` and the pause has
