...
Nota's existing risk classifier + audit log + approvals + Touch ID flow
all work unchanged. The only adapter-layer work is the field-rename
between Cursor's input/output JSON and Nota's internal `decide()` core.
...
Hook contract (verified at https://cursor.com/docs/hooks):
  ...
  Config at `~/.cursor/hooks.json`:
    {
      ...
      "hooks": {
        "beforeShellExecution": [
          { "command": "nota cursor-hook", "type": "command", "timeout": 30 }
        ]
      }
    }

  Stdin (Cursor → Nota, one JSON line):
    { "command": "rm -rf /",
      "cwd": "/repo",
      "sandbox": false,
      "conversation_id": "...",
  ...
  Stdout (Nota → Cursor, one JSON line):
    { "permission": "deny",
      "agent_message": "rm -rf is blocked by Nota policy",
...
the user has enabled Auto-Run mode. Nota returns `"deny"` (not `"ask"`)
...
from nota.config import default_audit_path
...
_EVENT_TOOL_NAME: dict[str, str] = {
    ...
    # observability hooks, not gate hooks - Nota ignores them by passing
    # through `allow`.
}


def _normalize_input(raw: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    """Cursor stdin → (tool_name, tool_input) for Nota's classifier.
...
def decide(tool_name: str, tool_input: Mapping[str, Any]) -> HookDecision:
    ...
    if base.permission == "ask":
        ...
        # user explicitly approves via `nota approve <token>` (the same
        ...
        return HookDecision(
            permission="deny",
            reason=body + " · approve via `nota approve <token>` to release",
            risk=risk,
...
def run_hook(stdin_text: str, audit: AuditLog | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "permission": "allow",
            "agent_message": f"nota: malformed cursor-hook input: {e}",
        }
    ...
    if session_id and session_id != "cursor":
        with contextlib.suppress(Exception):
            from nota.adapters.claude_code import _track_session
    ...
    if decision.permission == "allow" and not approval_token_used and session_id:
        with contextlib.suppress(Exception):
            from nota.adapters.claude_code import _taint_state_for
    ...
    if decision.permission != "allow":
        ...
        if decision.try_instead:
            response["user_message"] = (
                f"Nota blocked: {decision.what or tool_name}. Try: {decision.try_instead}"
            )
        else:
            response["user_message"] = f"Nota blocked: {decision.reason}"
...
def install_into_settings(settings_path: Any = None) -> tuple[Any, bool]:
    ...
    nota_entry = {"command": "nota cursor-hook", "type": "command", "timeout": 30}
    was_already = True
    for event_name in ("beforeShellExecution", "beforeMCPExecution", "beforeReadFile"):
        ...
        if not isinstance(existing_handlers, list):
            existing_handlers = []
        already = any(
            isinstance(h, dict) and h.get("command", "").startswith("nota ")
            for h in existing_handlers
        )
        if not already:
            existing_handlers.append(dict(nota_entry))
            was_already = False
...
def main() -> int:
    ...
    Wired to `nota cursor-hook` via the CLI module. Routes the audit
    log to <cwd>/.nota/audit.log.jsonl when the user has opted in to
    per-project mode (just like the Claude Code adapter).
    """
    from nota.adapters.claude_code import _default_load_hmac_key, _resolve_project_paths
    ...
    try:
        ...
        response = {
            "permission": "allow",
            "agent_message": f"nota: hook crashed (bug, fail-open): {e}",
        }
