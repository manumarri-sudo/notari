...
operator. The operator decides whether to apply them. Nota never
...
Suggestion classes:
  ...
  3. False-positive overrides - patterns whose blocks the operator has
     repeatedly bypassed via `nota approve`. Suggests adding a
...
KPIs (`nota kpis`) — three signals that genuinely measure whether
...
class KPIReport:
    ...
    n_overrides: int = 0  # operator-bypasses via nota approve
...
def _normalize_block_reason(reason: str) -> str:
    ...
    classifier versions emitted longer suffixes ("Nota blocked: rm
    ...
    s = reason
    # Old format prefix
    if s.startswith("Nota blocked: "):
        s = s[len("Nota blocked: ") :]
    ...
    for cwd, n in sorted(asks_by_cwd.items(), key=lambda kv: -kv[1]):
        ...
        out.append(
            Suggestion(
                ...
                paste_command=f"nota trust add {cwd}",
                evidence=(
                    f"asks={n}",
                    f"blocks={n_blocks}",
                ),
...
def analyze_decayed_permissions() -> list[Suggestion]:
    ...
    out: list[Suggestion] = []
    try:
        from nota import decay as _decay
    ...
    for perm in store.decayed():
        out.append(
            Suggestion(
                ...
                paste_command=f"nota decay reaffirm {perm.pattern}",
...
def analyze_false_positive_overrides(
    ...
    for e in events:
        ...
        # reason ends with "via nota approve <token-prefix>"; the
        ...
        out.append(
            Suggestion(
                ...
                rationale=(
                    ...
                    f"it via `nota approve` {n} times. Repeated overrides "
                ...
                paste_command=(
                    f'edit ~/.nota/config.toml and set: [policy]\n  "{tool_name}" = "medium"'
                ),
    ...
    for head, n in sorted(pattern_hits.items(), key=lambda kv: -kv[1]):
        ...
        out.append(
            Suggestion(
                ...
                paste_command=(
                    f"edit ~/.nota/config.toml and add: "
        ...
        out.append(
            Suggestion(
                ...
                rationale=(
                    ...
                    "src/nota/journal.py:summarize_transcript and confirm "
