...
from nota.policy import Scope, SessionIntent


@dataclass(slots=True)
class SubAgentExceedsParentScope(NotaError):
    """Raised when a child sub-agent declares a scope its parent doesn't have.

    Attenuation is one of the strongest guarantees Nota provides; violating
...
@dataclass(slots=True)
class SessionTree:
    """Owns the whole delegation tree for a single nota run.

    One per `nota serve` process. Hands out SessionNode handles and
