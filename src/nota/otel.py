...
When `nota[otel]` is installed AND OTel is configured (env var
...
Phoenix / Honeycomb ingest Nota data without inventing a new format.
...
- Optional extra (`nota[otel]`), not default-on. Footprint: 3
...
- `gen_ai.operation.name` = the audit event_type (e.g. "verdict.blocked")
  so observability dashboards can filter by Nota's own taxonomy too.

If the user doesn't `pip install 'nota[otel]'`, the import in this
...
# is visible. `nota doctor` surfaces the count.
...
def _try_load_otel() -> bool:
    ...
    try:
        ...
        _otel_tracer = trace.get_tracer("nota", "0.2.0")
    except ImportError:
        _otel_tracer = None
    return _otel_tracer is not None
...
# ---------------------------------------------------------------------------
# Mapping from Nota audit events to OTel GenAI span names + attributes
...
def _span_attributes(
    ...
    Plus Nota-specific attributes prefixed `nota.*` so dashboards
...
def _provider_name(agent_id: str) -> str:
    """Map Nota's agent_id taxonomy to a GenAI semconv provider name."""
    if agent_id.startswith("upstream:"):
        return agent_id.split(":", 1)[1]
    if agent_id.startswith("claude-code"):
        return "claude-code"
    if agent_id == "nota.notify" or agent_id.startswith("nota."):
        return "nota"
...
def emit_span(
    *,
    ...
    with (
        ...
        # relying on nota.event_type substring matching.
        if event_type in (
            "verdict.blocked",
            "verdict.scope_violation",
            "tool.errored",
