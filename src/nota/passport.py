"""The Nota Change Passport: the evidence artifact a verification produces.

A passport is a small, self-contained summary of one `nota verify` run - the
...
from nota import attest, explain
from nota.verify import Verdict, VerifyResult
...
def render_markdown(
    ...
    fix prompt, and what Nota does not prove), reusing the same remediation
    logic as `nota explain` so the PR surface is the action loop, not a raw
    evidence dump. The evidence/trust/audit detail stays lower in the document.
    """
    from nota import explain, teach
    ...
    lines: list[str] = []
    lines.append("# Nota Change Passport")
    lines.append("")
    ...
    if d["remediations"]:
        ...
        lines.append(
            ...
            "`passport.json` with `nota verify-passport`."
        )
        if signed:
            lines.append(
                "The JSON passport is gate-signed and can be verified with "
                "`nota verify-passport` — a reviewer can confirm this verdict "
    ...
    if result.gate_tamper_hits:
        ...
        lines.append("")
        lines.append("_This PR edits Nota's own trust surfaces — always a BLOCK:_")
...
def write_passport(
    ...
    *,
    ...
    with ``nota verify-passport`` without trusting the repo it came from.
    """
